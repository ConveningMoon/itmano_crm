...
export function buildNewsletterIntegrationPrompt(input: NewsletterIntegrationInput): string {
  ...
  const lines: string[] = [
    ...
    '  · `visitor_id` — un identificador estable del visitante. El formulario propio',
    '    del CRM lo guarda en `localStorage` bajo la clave `itmano_visitor_id` y la',
    '    comparte con `intake.js`. Es lo que une la vista medida (sección 3) con la',
    '    suscripción: usa la MISMA clave en tu web para que las dos superficies',
    '    cuenten al mismo visitante en vez de a dos personas distintas.',
    '',
    ...
    '`200`, que es el mismo en los dos casos. Cualquier otro código trae `error` con',
    'el motivo.',
    '',
    '### Qué pasa después',
    input.hasSequence
    ...
    '`keepalive` no siempre puede completar a tiempo. A cambio, la respuesta llega',
    '**opaca** — cross-origin, sin cabeceras CORS — así que no intentes leer su',
    'cuerpo ni su código: dispárala y olvídala (`.catch(() => {})`), igual que hace',
    'el propio CRM en sus páginas alojadas.',
    '',
