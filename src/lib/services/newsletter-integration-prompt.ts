...
export function buildNewsletterIntegrationPrompt(input: NewsletterIntegrationInput): string {
  ...
  const lines: string[] = [
    ...
    'contenido a alguien en vez de a la página.',
    '',
    'Antes de marcarla, mira QUÉ tipo le asignas: `author_name` no siempre es una',
    'persona. Una edición que ningún agente firma lleva el nombre de la agencia, y',
    'hoy ése es el caso más común. Compárala con el nombre de tu agencia: si',
    'coincide, márcala como `Organization`; si no, como `Person`. Marcar a la',
    'agencia como `Person` es una atribución falsa, y un buscador la trata como',
    'tal. Es exactamente lo que hace la propia página de ITMANO.',
    '',
