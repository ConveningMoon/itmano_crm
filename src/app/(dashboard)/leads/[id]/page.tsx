...
function generateMockEvents(lead: Lead): TimelineEvent[] {
  const events: TimelineEvent[] = []
  const base = new Date(lead.createdAt)
  const sourceName = getSourceById(lead.sourceId)?.name ?? 'fuente desconocida'
...
export default function LeadDetailPage() {
  ...
  // All hooks declared before conditional return
  ...
  const [localTempScore, setLocalTempScore] = useState(lead?.temperatureScore ?? 0)
  ...
  if (!lead) {
    return (
      <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '240px', gap: '12px' }}>
        <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>Lead no encontrado</p>
        <button
  ...
  const events    = generateMockEvents(lead)
