...
export function buildNewsletterIntegrationPrompt(input: NewsletterIntegrationInput): string {
  ...
  const lines: string[] = [
    ...
    '',
    'Cada una lleva SU tipo, y ya no hay que adivinarlo: `author_name` es siempre',
    'una persona y `author_org_name` siempre una organización. Marca la persona con',
    'marcado de autor (un `rel="author"`, un `<address>`, o el `author` de tu propio',
    'JSON-LD) en vez de como texto suelto: es la señal que un buscador necesita para',
    'atribuir el contenido a alguien en vez de a la página. La agencia va como',
    '`publisher` o como `Organization`, nunca como `Person`: eso es una atribución',
    'falsa y un buscador la trata como tal. Es exactamente lo que hace la propia',
    'página de ITMANO.',
    '',
